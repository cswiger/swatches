...
VMINT		g_tid;			/* global timer ID */
VMFLOAT		g_y	= 0;		/* angle for color rotation */
...
*  the process on screen when the VM_MSG_ACTIVE or VM_MSG_INACTIVE is received.
	*/
	...
	case VM_MSG_PAINT:	/* the message of asking for application to repaint screen */
		...
		g_tid = vm_create_timer(50, (VM_TIMERPROC_T)draw_hello);
...
static void draw_hello(void) {
	...
	vm_graphic_fill_rect_ex(layer_hdl[0], 0, 0, sw, sh);
	
	/* make swatches of color */
    r = (VMINT)((1+sin(g_y))*127);
	g = (VMINT)((1+sin(g_y+1.57))*127);
	b = (VMINT)((1+sin(g_y+3.14))*127);
    color.vm_color_565 = VM_RGB565_ARGB(128, r, g, b);
	vm_graphic_setcolor(&color);
	g_y += g_yinc;
	vm_graphic_fill_rect_ex(layer_hdl[0], 0, 0, sw/2, sh/2);

    r = (VMINT)((1+sin(g_y+1.57))*127);
	g = (VMINT)((1+sin(g_y+3.14))*127);
	b = (VMINT)((1+sin(g_y+4.71))*127);
    color.vm_color_565 = VM_RGB565_ARGB(128, r, g, b);
	vm_graphic_setcolor(&color);
	g_y += g_yinc;
	vm_graphic_fill_rect_ex(layer_hdl[0], sw/2, 0, sw/2, sh/2);

    r = (VMINT)((1+sin(g_y+4+3.14))*127);
	g = (VMINT)((1+sin(g_y+4.71))*127);
	b = (VMINT)((1+sin(g_y+6.28))*127);
    color.vm_color_565 = VM_RGB565_ARGB(128, r, g, b);
	vm_graphic_setcolor(&color);
	g_y += g_yinc;	vm_graphic_fill_rect_ex(layer_hdl[0], 0, sh/2, sw/2, sh/2);

    r = (VMINT)((1+sin(g_y+4.71))*127);
	g = (VMINT)((1+sin(g_y+6.28))*127);
	b = (VMINT)((1+sin(g_y+7.85))*127);
    color.vm_color_565 = VM_RGB565_ARGB(128, r, g, b);
	vm_graphic_setcolor(&color);
	g_y += g_yinc;	vm_graphic_fill_rect_ex(layer_hdl[0], sw/2, sh/2, sw/2, sh/2);
